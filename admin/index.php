...
switch ($do_action) {
    ...
    case "submit_hide_guest":
        ...
        if ($_GET["logs_hide_status"] == "true") {
            ...
            if ($_GET["logs_hide_status"] == "false") {
    ...
    case "save_set_allgemein":
        ...
        if (check_email($email_info)) {
            ko_set_setting('info_email', $email_info);
        }
    ...
    case 'submit_sms_sender_id':
        if (!ko_module_installed('sms') || $SMS_PARAMETER['provider'] != 'aspsms' || !$SMS_PARAMETER['user'] || !$SMS_PARAMETER['pass']) {
            continue;
        }
    ...
    case "save_set_layout":
        ...
        foreach ($FRONTMODULES as $fm_i => $fm) {
            if (!ko_check_fm_for_user($fm_i, $uid)) {
                continue;
            }
        ...
        ko_save_userpref($uid, "save_files_as_share",
            format_userinput($_POST["sel_save_files_as_share"], "uint", false, 1));
        ...
        ko_save_userpref($uid, 'download_not_directly',
            format_userinput($_POST['download_not_directly'], 'uint', false, 1));

        ko_set_setting('guest_portal_note', format_userinput($_POST['txt_guest_portal_note'], 'text'));
    ...
    case "submit_edit_admingroup":
        ...
        if (in_array("leute", $save_modules)) {  //Leute-Rechte
            ...
            $filterset = array_merge((array)ko_get_userpref('-1', '', 'filterset'),
                (array)ko_get_userpref($_SESSION['ses_userid'], '', 'filterset'));
            for ($i = 1; $i < 4; $i++) {
                ...
                if ($filter == -1) {
                    ...
                    if ($filter == "") {
                        ...
                        if (preg_match('/sg[0-9]{4}/', $filter) == 1) {  //small group
                            $sg = db_select_data('ko_kleingruppen',
                                "WHERE `id` = '" . format_userinput($filter, 'uint') . "'", '*', '', '', true);
                            $sgFilter = db_select_data('ko_filter', "WHERE `typ` = 'leute' AND `name` = 'smallgroup'",
                                'id', '', '', true);
                            $save_filter[$i]['value'] = $filter;
                            $save_filter[$i]['name'] = $sg['name'];
                            $save_filter[$i]['filter'] = array(
                                'link' => 'and',
                                0 => array(0 => $sgFilter['id'], 1 => array(1 => $sg['id']), 2 => 0)
                            );
                        } else {
                            if (preg_match('/g[0-9]{6}/', $filter) == 1) {  //group
                                $gid = substr($filter, -6);
                                $gr = db_select_data('ko_groups', "WHERE `id` = '$gid'", '*', '', '', true);
                                $grFilter = db_select_data('ko_filter', "WHERE `typ` = 'leute' AND `name` = 'group'",
                                    'id', '', '', true);
                                $save_filter[$i]['value'] = $filter;
                                $save_filter[$i]['name'] = $gr['name'];
                                $save_filter[$i]['filter'] = array(
                                    'link' => 'and',
                                    0 => array(0 => $grFilter['id'], 1 => array(1 => $filter, 2 => ''), 2 => 0)
                                );
                            } else {  //filter preset
            ...
            $presets = array_merge((array)ko_get_userpref('-1', '', 'leute_itemset', 'ORDER BY `key` ASC'),
                (array)ko_get_userpref($_SESSION['ses_userid'], '', 'leute_itemset', 'ORDER BY `key` ASC'));
            //view
            ...
            if (sizeof($save_preset) == 0) {
                ...
                if ($save_preset["view"]) {
                    $save_preset["view"] = array_unique(array_merge((array)$save_preset["view"],
                        (array)$save_preset["edit"]));
                }
            }
            ...
            if (in_array('leute', explode(',', $old_admingroup['modules']))) {
        ...
        if (in_array('groups', $save_modules)) {
            ...
            for ($i = 4; $i > 0; $i--) {
                if (isset($_POST["sel_groups_rights_" . $mode[$i]])) {
                    ...
                    $old = explode(",",
                        format_userinput($_POST["old_sel_groups_rights_" . $mode[$i]], "intlist", false, 0, array(),
                            ":"));
                    $new = explode(",",
                        format_userinput($_POST["sel_groups_rights_" . $mode[$i]], "intlist", false, 0, array(), ":"));
                    ...
                    foreach ($deleted as $gid) {
                        ...
                        foreach ($rights_array as $index => $right) {
                            if ($right == 'g' . $id) {
                                unset($rights_array[$index]);
                            }
                        }
                        foreach ($rights_array as $a => $b) {
                            if (!$b) {
                                unset($rights_array[$a]);
                            }
                        }  //Leere Eintr�ge l�schen
                        //Neuer Eintrag in Gruppe speichern
                        db_update_data("ko_groups", "WHERE `id` = '$gid'",
                            array("rights_" . $mode[$i] => implode(",", $rights_array)));
            ...
            if (in_array('groups', explode(',', $old_admingroup['modules']))) {
                ...
                foreach (array('view', 'new', 'edit', 'del') as $amode) {
                    $granted_groups = db_select_data('ko_groups',
                        "WHERE `rights_" . $amode . "` REGEXP '(^|,)g$id(,|$)'");
                    foreach ($granted_groups as $gg) {
                        $granted_logins = explode(',', $gg['rights_' . $amode]);
                        foreach ($granted_logins as $k => $v) {
                            if ($v == 'g' . $id) {
                                unset($granted_logins[$k]);
                            }
                        }
                        db_update_data('ko_groups', "WHERE `id` = '" . $gg['id'] . "'",
                            array('rights_' . $amode => implode(',', $granted_logins)));
        ...
        foreach ($MODULES_GROUP_ACCESS as $module) {
            ...
            if (in_array($module, $save_modules)) {
                ...
                switch ($module) {
                    case "daten":
                        if (ko_get_setting('daten_access_calendar') == 1) {
                            ...
                            $egs = db_select_data('ko_eventgruppen', "WHERE `calendar_id` = '0'", '*',
                                'ORDER BY name ASC');
                            foreach ($egs as $eid => $eg) {
                                $gruppen[$eid] = $eg;
                            }
                        } else {
                        ...
                        ko_save_admin($module . '_force_global', $id, $_POST['sel_force_global_' . $module],
                            "admingroups");
                        ko_save_admin($module . '_reminder_rights', $id, $_POST['sel_reminder_rights_' . $module],
                            "admingroups");
                        break;
                    case "reservation":
                ...
                foreach ($gruppen as $g_i => $g) {
                    $save_string .= format_userinput($_POST["sel_rechte_" . $module . "_" . $g_i], "uint", false,
                            1) . "@" . $g_i . ",";
                }
            } else {
                $save_string = "0 ";
        ...
        foreach ($MODULES as $module) {
            if (in_array($module, $done_modules)) {
                continue;
            }
            $done_modules[] = $module;
    ...
    case "submit_edit_login":
        ...
        if ($id == ko_get_root_id() && $_SESSION["ses_username"] != "root") {
            continue;
        }
        ...
        if ($_POST["txt_name"] != "") {
            ...
            if ($id != ko_get_guest_id() && $id != ko_get_root_id()) {
                ko_save_admin("login", $id, $login_name);
            }
        } else {
        ...
        foreach ($save_admingroups as $m_i => $m) {
            if (!in_array($m, array_keys($admingroups))) {
                unset($save_admingroups[$m_i]);
            }
        }
        ...
        $log_message .= 'Module: "' . implode(", ", $save_modules) . '", Admingroups: "' . implode(", ",
                $save_admingroups) . '", ';
        ...
        if (in_array("leute", $save_modules)) {  //Leute-Rechte
            ...
            if (in_array('leute', explode(',', $old_login['modules']))) {
        ...
        if (in_array('groups', $save_modules)) {
            ...
            if (in_array('groups', explode(',', $old_login['modules']))) {
                //If groups module has been deselected then remove all access settings from ko_groups
                foreach (array('view', 'new', 'edit', 'del') as $amode) {
                    $granted_groups = db_select_data('ko_groups',
                        "WHERE `rights_" . $amode . "` REGEXP '(^|,)$id(,|$)'");
                    foreach ($granted_groups as $gg) {
                        $granted_logins = explode(',', $gg['rights_' . $amode]);
                        foreach ($granted_logins as $k => $v) {
                            if ($v == $id) {
        ...
        if ($old_login['modules'] == implode(',', $save_modules)) {
            $_SESSION['show'] = 'show_logins';
        }
        break;
    ...
    case "submit_neues_login":
        ...
        if (!$notifier->hasErrors()) {
            ...
            if ($copy_userprefs_id) {
                $userprefs = db_select_data("ko_userprefs", "WHERE `user_id` = '$copy_userprefs_id'",
            ...
            if ($copy_rights_id) {
                foreach (array('rights_view', 'rights_new', 'rights_edit', 'rights_del') as $right) {
                    ...
                    foreach ($groups as $group) {
                        db_update_data("ko_groups", "WHERE `id` = '" . $group["id"] . "'",
                            array($right => ($group[$right] . "," . $id)));
                    }
                }
            }
    ...
    case "delete_login":
        ...
        if ((int)$id == (int)ko_get_guest_id()) {
            continue;
        }  //ko_guest may not be deleted
        if ((int)$id == (int)ko_get_root_id()) {
            continue;
        }   //root may not be deleted
    ...
    case "open_etiketten":
        ...
        //Auf g�ltige Vorlagen-ID pr�fen
    ...
    case "submit_del_etiketten_vorlage":
        ...
        if (is_array($vorlage) && $vorlage["name"] != "") {
            ...
            if ($vorlage['pic_file'] && file_exists($BASE_PATH . $vorlage['pic_file'])) {
                unlink($BASE_PATH . $vorlage['pic_file']);
            }
    ...
    case "save_etiketten":
        ...
        if ($_POST["txt_vorlage_neu"]) {  //Neue Vorlage speichern
            ...
            if ($_POST["sel_vorlage_save"]) {  //Bestehende neu speichern
        ...
        //Verlangte Angaben �berpr�fen (per_row, per_col)
    ...
    case "submit_edit_leute_pdf":
        ...
        if (!$notifier->hasErrors()) {
            ...
            foreach (array("left", "top", "right", "bottom") as $pos) {
                $new["page"]["margin_" . $pos] = $post["page"]["margin_" . $pos];
            }
            ...
            if ($post['columns']) {
                if (substr($post['columns'], 0, 3) == '@G@') {
                    $value = ko_get_userpref('-1', substr($post['columns'], 3), 'leute_itemset');
                } else {
                    $value = ko_get_userpref($_SESSION['ses_userid'], $post['columns'], 'leute_itemset');
                }
            ...
            if ($post["filter"]) {
                if (substr($post['filter'], 0, 3) == '@G@') {
                    ...
                    $value = ko_get_userpref($_SESSION['ses_userid'], '', 'filterset');
                }
                foreach ($value as $v_i => $v) {
                    if ($v["key"] == $post["filter"]) {
                        $new["filter"] = unserialize($value[$v_i]["value"]);
                    }
                }
            }
            ...
            if ($do_action == "submit_edit_leute_pdf") {
                db_update_data('ko_pdf_layout', "WHERE `id` = '$layout_id'",
                    array('name' => $name, 'data' => serialize($new)));
            } else {
    ...
    case 'sms_log_mark':
        ...
        db_insert_data('ko_log',
            array('type' => 'sms_mark', 'user_id' => $_SESSION['ses_userid'], 'date' => date('Y-m-d H:m:i')));
        break;
...
if (!$_SESSION['sort_logins']) {
    $_SESSION['sort_logins'] = 'login';
}
if (!$_SESSION['sort_logins_order']) {
    $_SESSION['sort_logins_order'] = 'ASC';
}
if (!$_SESSION["sort_logs"]) {
    $_SESSION["sort_logs"] = "date";
}
if (!$_SESSION["sort_logs_order"]) {
    $_SESSION["sort_logs_order"] = "DESC";
}
...
if (!$_SESSION["show_limit"]) {
    $_SESSION["show_limit"] = ko_get_setting("show_limit_logins");
}
if (!$_SESSION["show_logs_start"]) {
    $_SESSION["show_logs_start"] = 1;
}
$_SESSION["show_logs_limit"] = ko_get_userpref($_SESSION["ses_userid"], "show_limit_logs");
if (!$_SESSION["show_logs_limit"]) {
    $_SESSION["show_logs_limit"] = ko_get_setting("show_limit_logs");
}
...
if (!$_SESSION['sort_news']) {
    $_SESSION['sort_news'] = 'cdate';
}
if (!$_SESSION['sort_news_order']) {
    $_SESSION['sort_news_order'] = 'DESC';
}
...
print ko_include_css();
    ...
    $js_files = [
        ...
        FLOCKR_baseUrl. 'Modules/Legacy/Resources/Public/Scripts/kOOL.js',
        FLOCKR_baseUrl. 'inc/ckeditor/ckeditor.js',
        FLOCKR_baseUrl. 'inc/ckeditor/adapters/jquery.js',
        FLOCKR_baseUrl. 'Modules/Core/Resources/Public/Scripts/FormUI.js',
...
ko_get_submenu_code("admin", "left");
ko_get_submenu_code("admin", "right");

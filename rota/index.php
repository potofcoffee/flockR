...
$legacyApp = new \Peregrinus\Flockr\Legacy\LegacyApp('rota', '../');
...
switch ($do_action) {
    ...
    case 'submit_rota_settings':
        ...
        //User settings
        ko_save_userpref($_SESSION['ses_userid'], 'default_view_rota',
            format_userinput($_POST['sel_rota_default_view'], 'js'));
        ...
        if ($access['rota']['MAX'] > 2) {
            ...
            ko_save_userpref($_SESSION['ses_userid'], 'rota_schedule_subgroup_members',
                format_userinput($_POST['schedule_subgroup_members'], 'uint'));
        }
        if ($access['rota']['MAX'] > 1) {
            ko_save_userpref($_SESSION['ses_userid'], 'rota_pdf_fontsize',
                format_userinput($_POST['pdf_fontsize'], 'uint'));
            ko_save_userpref($_SESSION['ses_userid'], 'rota_pdf_use_colors',
                format_userinput($_POST['pdf_use_colors'], 'uint'));
        }
        ko_save_userpref($_SESSION['ses_userid'], 'rota_eventfields',
            format_userinput($_POST['eventfields'], 'alphanumlist'));

        //Global settings
        if ($access['rota']['MAX'] > 4) {
            ko_set_setting('rota_showroles', format_userinput($_POST['showroles'], 'uint'));
            if ($_POST['showroles'] == 1) {
                ko_set_setting('rota_teamrole', '');
            } else {
                ko_set_setting('rota_teamrole', format_userinput($_POST['teamrole'], 'uint'));
            }
    ...
    case 'submit_new_team':
        ...
        if (!$notifier->hasErrors() && $new_id) {
            ...
            if ($new_team['rotatype'] == 'week' && ko_get_setting('rota_export_weekly_teams') == 1) {
                $egid = db_insert_data('ko_eventgruppen', array(
                    'calendar_id' => ko_get_setting('rota_export_calid'),
                    'name' => $new_team['name'],
                    'type' => 2
                ));
    ...
    case 'delete_team':
        ...
        if ($access['rota']['ALL'] < 5 && $access['rota'][$id] < 5) {
            continue;
        }
    ...
    case 'multiedit':
        ...
        foreach ($_POST['chk'] as $c_i => $c) {
            if ($c) {
                ...
                if ($access['rota']['ALL'] > 2 || $access['rota'][$edit_id] > 2) {
                    $do_ids[] = $edit_id;
                }
            }
        }
    ...
    case 'filesend':
        ...
        if (!ko_get_leute_email($p, $emails)) {
            $emails = array(ko_get_setting('info_email'));
        }

        //Check for valid sender address
        $from_email = format_userinput($_POST['sender'], 'email');
        if (!check_email($from_email) || !in_array($from_email, $emails)) {
            continue;
        }
        ...
        if (in_array($_POST['recipients'], array('schedulled', 'selectedschedulled', 'manualschedulled'))) {
            ...
            if ($_POST['recipients'] == 'selectedschedulled') {
                ...
                if ($_POST['recipients'] == 'manualschedulled') {
            ...
            if ($_POST['recipients'] == 'single') {
                ...
                foreach ($teams as $team) {
                    if ($access['rota']['ALL'] < 4 && $access['rota'][$team['id']] < 4) {
                        continue;
                    }
        ...
        if ($_POST['recipients_group']) {
            ...
            if ($gid) {
                ...
                if ($group['id'] > 0 && $group['id'] == $gid) {
                    ...
                    $group_members = db_select_data('ko_leute',
                        "WHERE `deleted` = '0' AND `hidden` = '0' AND `groups` LIKE '%g$gid%'");
        ...
        if ($_POST['save_preset']) {
            ...
            ko_save_userpref($uid, format_userinput($_POST['save_preset'], 'js'), $_POST['text'],
                'rota_emailtext_presets');
        }
        ...
        ko_log('rota_sendfile',
            'From: ' . $from_email . ', Subject: ' . $subject . ', Raw text: ' . $_POST['text'] . ', File: ' . $filetype . ' (' . $filename . '), Recipients type: ' . $_POST['recipients'] . ', Recipients: ' . implode(', ',
                $email_recipients) . ', No Email: ' . implode(', ', $noemail_recipients));
...
if (!isset($_SESSION['rota_teams']) || $_SESSION['rota_teams'] == '') {
    ...
    foreach ($_SESSION['rota_teams'] as $k => $v) {
        if ($v == '') {
            unset($_SESSION['rota_teams'][$k]);
        }
    }
}
...
$js_files = [
    ...
    '//cdnjs.cloudflare.com/ajax/libs/ckeditor/4.6.0/ckeditor.js',
    '//cdnjs.cloudflare.com/ajax/libs/ckeditor/4.6.0/adapters/jquery.js',
...
print ko_include_js($js_files);
    ...
    include(FLOCKR_basePath. 'rota/inc/js-rota.inc');
...
ko_get_submenu_code("rota", "left");
ko_get_submenu_code("rota", "right");
        ...
        <form action="index.php" method="post" name="formular" enctype="multipart/form-data">
                ...
                <div name="main_content" id="main_content">
                    ...
                    switch ($_SESSION['show']) {
                        ...
                        case 'multiedit':
                            ko_multiedit_formular('ko_rota_teams', $do_columns, $do_ids, $order,
                                array('cancel' => 'list_teams'));
                            break;

				...
				foreach($reservations as $res) {
					...
					if($i == 1) {
						if(!ko_res_check_double($res['item_id'], sql2datum($res['startdatum']), sql2datum($res['enddatum']), substr($res['startzeit'], 0, -3), substr($res['endzeit'], 0, -3), $double_error)) {
							$title = '! '.$title.' !';
							$tooltip .= '<b>'.getLL('res_collision_text').'</b><br />'.utf8_encode($double_error).'<br /><br />';
							$checkLink = FALSE;
						} else {
					...
					//Add links to approve or delete a moderation
						...
						if($checkLink) {
							$checkIcon = '<input type="image" src="../images/button_check.png" title="'.getLL('res_mod_confirm').'" onclick="c1=confirm(\''.utf8_encode(getLL('res_mod_confirm_confirm')).'\');if(!c1) return false; c=confirm(\''.utf8_encode(getLL('res_mod_confirm_confirm2')).'\');set_hidden_value(\'id\', \''.$res['id'].'\', this);set_hidden_value(\'mod_confirm\', c, this);set_action(\'res_mod_approve\', this);" />';
						} else {
							$checkIcon = '';
						}

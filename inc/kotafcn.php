...
/**
  * Liefert Select-Werte f�r einzelne Tabelle-Spalten
	*/
...
/**
  * Speichert die �nderungen von einem Multiedit-Formular
	*/
function kota_submit_multiedit($rights_level='', $log_type="", $lastchange_col="", &$changes) {
	...
	foreach($koi as $col => $values) {
		...
		//Spalten mit _PLUS hinten geh�ren zu einem Textplus-Feld, werden deshalb schon bearbeitet (in kota_process_data())
		...
		foreach($values as $id => $value) {
			...
			//Query aufbauen, aber noch nicht ausf�hren
	...
	//IDs pr�fen, die nun nicht mehr vorkommen.
	//Es k�nnte sich um Checkboxen handeln, die deaktiviert wurden, denn dann wird der Wert von HTML gar nicht gesetzt
	...
	//�bereinstimmung mit Hash-Wert checken
	sort($test["columns"]);
	sort($test["ids"]);
...
/**
  * Funktionen zur Nach-Behandlung (post) einzelner Formularfelder f�r Multiedit
	*/
...
//	\Peregrinus\Flockr\Core\Debugger::dumpAndDie([$table, $data, $modes, $log, $id]);
	...
	foreach($data as $col => $value) {
		...
		//Bei Textplus-feldern auf Wert im Text-feld �berpr�fen und diesen verwenden falls != ""

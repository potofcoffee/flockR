...
if(isset($_GET) && isset($_GET["action"])) {
	...
	switch($action) {
		...
		case "leutedelfilterset":
			...
			if($akt_filter['_name'] == 'group') print "@@@POST@@@filter_group;$('#sm_leute_filter h5 a').first().click();"; else print "@@@POST@@@$('#sm_leute_filter h5 a').first().click();";
		break;
		...
		case "leuteopenfilterset":
			...
			//Filter l�schen, falls Neu Anwenden geklickt wurde
			...
			//Neuen HTML-Code f�r Main ausgeben
			print "main_content@@@";
			...
			//group filter
			ko_get_filter_by_id($_SESSION["filter_akt"], $akt_filter);
			if($akt_filter["sql1"] == "groups REGEXP '[VAR1][g:0-9]*[VAR2]'") print "@@@POST@@@filter_group;$('#sm_leute_filter h5 a').first().click();"; else
				print "@@@POST@@@$('#sm_leute_filter h5 a').first().click();";
		...
		case "itemlist":
			...
			if($_SESSION["show"] == "list_kg") {
				...
				//die ('<pre>'.print_r(ko_get_leute_col_name(FALSE, TRUE)));
				if($state == "checked") {  //Select it

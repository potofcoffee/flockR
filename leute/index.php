...
if ($_SESSION['leute_export_xls_post'] != '' && $_POST['action'] == 'leute_submit_export_xls' && $_SESSION['show'] == 'xls_settings') {
    ...
    if ($_POST['sel_children_columns'] != '') {
        ko_save_userpref($_SESSION['ses_userid'], 'leute_children_columns',
            format_userinput($_POST['sel_children_columns'], 'alphanumlist'));
    }
    ...
    ko_save_userpref($_SESSION['ses_userid'], 'leute_linebreak_columns',
        format_userinput($_POST['sel_linebreak_columns'], 'alphanumlist'));
    ko_save_userpref($_SESSION['ses_userid'], 'export_table_format',
        format_userinput($_POST['export_table_format'], 'alphanum'));
} else {
    unset($_SESSION['leute_export_xls_post']);
}
...
if (false === format_userinput($do_action, "alphanum+", true, 50)) {
...
switch ($do_action) {
    ...
    case "show_groupsubscriptions":
        if (isset($_GET['gid'])) {
            if (!$_GET['gid']) {
                unset($_SESSION['leute_gs_filter']);
            } else {
                $_SESSION['leute_gs_filter'] = format_userinput($_GET['gid'], 'uint');
            }
        }
    ...
    case "add_to_my_list":
        foreach (explode(',', $_POST['ids']) as $c) {
            if ($c) {
                ...
                if ($value && !in_array($value, $_SESSION['my_list'])) {
                    $_SESSION['my_list'][$value] = $value;
                }
            }
        }
    ...
    case "submit_als_neue_person":
        //Beim Editieren auf korrekte Leute-ID pr�fen
        if ($do_action == "submit_edit_person") {
            if (!$_POST["leute_id"] || !format_userinput($_POST["leute_id"], "uint", true, 10)) {
                continue;
            }
            ...
            if ($do_action == "submit_als_neue_person" || $do_action == "submit_neue_person") {
                //Create a new entry and store new id
                db_insert_data("ko_leute",
                    array("id" => "NULL", "crdate" => date("Y-m-d H:i:s"), "cruserid" => $_SESSION["ses_userid"]));
        ...
        //Funktion (kundenspezifisch) einlesen, die Variablen speziell behandeln l�sst
        ...
        if (is_array($allowed_cols["view"]) && sizeof($allowed_cols["view"]) > 0) {
            if ($access['groups']['MAX'] > 0) {
                $allowed_cols["view"][] = "groups";
            }
            if ($access['kg']['MAX'] > 1) {
                $allowed_cols["view"][] = "smallgroups";
            }
        }
        if (is_array($allowed_cols["edit"]) && sizeof($allowed_cols["edit"]) > 0) {
            if ($access['groups']['MAX'] > 1) {
                $allowed_cols["edit"][] = "groups";
            }
            if ($access['kg']['MAX'] > 2) {
                $allowed_cols["edit"][] = "smallgroups";
            }
        }
        ...
        if (!is_array($allowed_cols["edit"]) || in_array("famid", $allowed_cols["edit"])) {
            $do_update_familie = false;  //Familiendaten - falls n�tig - erst nach Speichern der Person updaten
            $new_familie = false;
            ...
            if ($save_famid != $old_famid) {
                ...
                //Alte Familie l�schen, falls keine Mitglieder mehr
                if ($old_famid != 0) {
                    $num = ko_get_personen_by_familie($old_famid, $asdf);
                    if ($num <= 1) {
                        db_update_data('ko_leute', "WHERE `famid` = '$old_famid'",
                            array('famid' => '0', 'kinder' => '0'));
        ...
        foreach ($leute_cols as $c_i => $c) {
            ...
            if (is_array($allowed_cols["edit"])) {
                if (!in_array($c["Field"], $allowed_cols["edit"])) {
                    continue;
                }
            } else {
                if (is_array($allowed_cols['view'])) {
                    if (!in_array($c["Field"], $allowed_cols["view"])) {
                        continue;
                    }
                }
            }
            ...
            if ($c["Field"] == "groups") {
                ...
                ko_groups_save_datafields($_POST["group_datafields"],
                    array("id" => $leute_id, "groups" => $_POST["input_groups"], "old_groups" => $person["groups"]),
                    $log2);

                //Log-Message:
                $dont_log = true;
                if (trim($log) != "") {
                    $log_message .= getLL("leute_log_groups") . ": $log";
                }
                if (trim($log2) != "") {
                    $log_message .= getLL("leute_log_datafields") . ": $log2";
                }
            }//groups-Modul
            ...
            switch ($input_type) {
                ...
                case "text":
                    if (in_array($c["Field"], $LEUTE_TEXTSELECT) && $_POST["input_" . $c["Field"] . "_2"]) {
                        ...
                        if (in_array($c["Field"], $LEUTE_ENUMPLUS) && $_POST["input_" . $c["Field"] . "_2"]) {
                            ...
                            db_alter_table("ko_leute",
                                "CHANGE `" . $c["Field"] . "` `" . $c["Field"] . "` ENUM( $enum_code ) DEFAULT NULL ");

                            if (function_exists("my_enumplus")) {
                                my_enumplus($c["Field"], format_userinput($_POST["input_" . $c["Field"]], "text"),
                                    format_userinput($_POST["input_" . $c["Field"] . "_2"], "text"));
                            }

                            $data[$c["Field"]] = format_userinput($_POST["input_" . $c["Field"] . "_2"], "text");
                        } else {
                            if ($KOTA['ko_leute'][$c['Field']]['form']['type'] == 'peoplesearch') {
                ...
                case "tinytext":
                    if ($_FILES["input_" . $c["Field"]]["tmp_name"]) {
                        ...
                        if ($_POST["input_" . $c["Field"] . "_DELETE"] == 1) {
                            $data[$c["Field"]] = "";
                            if ($person[$c["Field"]] && file_exists($BASE_PATH . $person[$c["Field"]])) {
                                unlink($BASE_PATH . $person[$c["Field"]]);
                            }
                        }
                    }
                    $test["columns"][] = $col;
            ...
            if (function_exists("my_fcn_leute")) {
                my_fcn_leute($c["Field"], format_userinput($_POST["input_" . $c["Field"]], "text"),
                    format_userinput($_POST["input_" . $c["Field"] . "_2"], "text"));
            }
            ...
            if (!$dont_log) {
                ...
                } else {  //Bei Editieren nur die loggen, die ge�ndert wurden
                    $new = format_userinput($_POST["input_" . $c["Field"]], "text");
                    $new2 = format_userinput($_POST["input_" . $c["Field"] . "_2"], "text");
                    $old = $c["Type"] == "date" ? map_leute_daten($person[$c["Field"]], $c["Field"],
                        $person) : $person[$c["Field"]];
                    if (($old || $new) && $new != $old) {
        ...
        //In DB speichern
        $data["lastchange"] = date("Y-m-d H:i:s");  //LastChange hinzuf�gen
        //Familien-ID hinzuf�gen
        if (!is_array($allowed_cols["edit"]) || in_array("famid", $allowed_cols["edit"])) {
            $data["famid"] = $save_famid;
            if ($data['famid'] <= 0) {
                $data['kinder'] = '0';
            }  //Set number of children to 0 for persons without a family
        }
        ...
        //Familien-Daten
        //Erst nachher updaten, damit Personendaten nicht Familiendaten �berschreiben
        if ($save_famid > 0 || $old_famid > 0) {
            if ($do_update_familie) {
        ...
        if (sizeof($LEUTE_EMAIL_FIELDS) > 1) {
            foreach ($LEUTE_EMAIL_FIELDS as $email_field) {
                if (is_array($allowed_cols['edit']) && !in_array($email_field, $allowed_cols['edit'])) {
                    continue;
                }
                $current = db_select_data('ko_leute_preferred_fields',
                    "WHERE `type` = 'email' AND `lid` = '$leute_id' AND `field` = '$email_field'", '*', '', '', true);
                if ($_POST['email_chk_' . $email_field]) {  //Field selected
                    if ($data[$email_field]) {  //Only set checkbox if email address is given
                        if (!$current) {
                            db_insert_data('ko_leute_preferred_fields',
                                array('type' => 'email', 'lid' => $leute_id, 'field' => $email_field));
                        }
                    } else {  //Delete checkbox if no email address is given
                        if ($current) {
                            db_delete_data('ko_leute_preferred_fields',
                                "WHERE `type` = 'email' AND `lid` = '$leute_id' AND `field` = '$email_field'");
                        }
                    }
        ...
        if (sizeof($LEUTE_MOBILE_FIELDS) > 1) {
            foreach ($LEUTE_MOBILE_FIELDS as $mobile_field) {
                if (is_array($allowed_cols['edit']) && !in_array($mobile_field, $allowed_cols['edit'])) {
                    continue;
                }
                $current = db_select_data('ko_leute_preferred_fields',
                    "WHERE `type` = 'mobile' AND `lid` = '$leute_id' AND `field` = '$mobile_field'", '*', '', '', true);
                if ($_POST['mobile_chk_' . $mobile_field]) {  //Field selected
                    if ($data[$mobile_field]) {  //Only set checkbox if mobile number is given
                        if (!$current) {
                            db_insert_data('ko_leute_preferred_fields',
                                array('type' => 'mobile', 'lid' => $leute_id, 'field' => $mobile_field));
                        }
                    } else {  //Delete checkbox if no mobile number is given
                        if ($current) {
                            db_delete_data('ko_leute_preferred_fields',
                                "WHERE `type` = 'mobile' AND `lid` = '$leute_id' AND `field` = '$mobile_field'");
                        }
                    }
                } else {  //Field not selected
                    if ($current) {
        ...
        if ($action != 'submit_neue_person' && defined("EXPORT2EZMLM") && EXPORT2EZMLM) {
            ...
            if ($old_email != $new_email && check_email($new_email)) {  //Check for changed valid email
                foreach (explode(",",
                    $data["groups"]) as $group) {  //Check this user's groups for one with an ML assigned
                    $gid = ko_groups_decode($group, "group_id");
                    if ($all_groups[$gid]["ezmlm_list"]) {
                        //Un- and resubscribe
                        ko_ezmlm_unsubscribe($all_groups[$gid]["ezmlm_list"], $all_groups[$gid]["ezmlm_moderator"],
                            $old_email);
                        ko_ezmlm_subscribe($all_groups[$gid]["ezmlm_list"], $all_groups[$gid]["ezmlm_moderator"],
                            $new_email);
                    }
                }
            }
            ...
            if ($log_message != "") {
                ...
                if (sizeof($_POST['sel_announce_changes']) > 0) {
                    ...
                    foreach ($_POST['sel_announce_changes'] as $lid) {
                        ...
                        $text = sprintf(getLL('leute_announce_changes_email_text'),
                                $sender['vorname'] . ' ' . $sender['nachname'], $name) . "\n\n" . str_replace(',', "\n",
                                substr($log_message, 0, -2));
                        ko_send_mail(array($sender['email'] => $sender['vorname'] . ' ' . $sender['nachname']),
                            $person['email'], getLL('leute_announce_changes_email_subject'), $text);
                    }
    ...
    case "edit_person":
        if ($action_mode == 'POST') {
            $leute_id = format_userinput($_POST['id'], 'uint');
        } else {
            if ($action_mode == 'GET') {
                $leute_id = format_userinput($_GET['id'], 'uint');
            }
        }
        if ($access['leute']['ALL'] > 1 || $access['leute'][$leute_id] > 1 || (ko_get_setting('login_edit_person') == 1 && $leute_id == ko_get_logged_in_id())) {
        } else {
            continue;
        }
        ...
        if (!is_array($allowed_cols["edit"]) || in_array("famid", $allowed_cols["edit"])) {
            ...
            foreach ($_POST["chk"] as $c_i => $c) {
                if ($c) {
                    ...
                    if ($access['leute']['ALL'] > 0 || $access['leute'][$edit_id] > 0) {
                        $do_ids[] = $edit_id;
                    }
                }
            }
    ...
    case 'merge_duplicates':
        ...
        foreach ($ids as $id => $value) {
            if (!in_array($id, $do_ids)) {
                continue;
            }  //Only test marked ids
            if (in_array($id, $done)) {
                continue;
            }  //Don't check IDs which have been added as duplicate of another ID
            foreach ($test as $tid => $t) {
                if (!in_array($tid, $do_ids)) {
                    continue;
                }  //Only test marked ids
                if ($t == $value) {
                    $duplicates[$c][] = $tid;
        ...
        foreach ($duplicates as $ids) {
                ...
                foreach ($person as $k => $v) {
                    ...
                    if ($k == 'groups') {
                        ...
                        if ($k == 'smallgroups') {
                            ...
                            if ((string)$v != '' && (string)$v != '0' && (string)$v != '0000-00-00') {
                                $num_fields[$id]++;
                            }
                        }
                    }
            ...
            foreach ($num_fields as $id => $num) {
                if ($first) {
                    ...
                    db_update_data('ko_leute', "WHERE `id` = '$id'", array(
                                ...
                                if ($data['value'] != $mdata['value']) {
                                    if ($mdata['value'] == '') {  //Entry of kept person is empty so use the value from the double entry
                                        db_update_data('ko_groups_datafields_data',
                                            "WHERE `group_id` = '" . $mdata['group_id'] . "' AND `datafield_id` = '" . $mdata['datafield_id'] . "' AND `person_id` = '" . $mdata['person_id'] . "'",
                                            array('value' => $data['value']));
                                    } else {
                                        if ($data['value'] == '') {  //Entry of the double entry is empty so keep the other
                                            //Do nothing
                                            ...
                                            if ($all_datafields[$data['datafield_id']]['type'] == 'multiselect') {
                                                db_update_data('ko_groups_datafields_data',
                                                    "WHERE `group_id` = '" . $mdata['group_id'] . "' AND `datafield_id` = '" . $mdata['datafield_id'] . "' AND `person_id` = '" . $mdata['person_id'] . "'",
                                                    array(
                                                        'value' => array_unique(array_merge(explode(',',
                                                            $mdata['value']), explode(',', $data['value'])))
                                                    ));
                                            } else {
                                                if ($all_datafields[$data['datafield_id']]['type'] == 'select') {
                                                    //Do nothing with different select values
                                                } else {
                                                    //Concatenate values with ,
                                                    db_update_data('ko_groups_datafields_data',
                                                        "WHERE `group_id` = '" . $mdata['group_id'] . "' AND `datafield_id` = '" . $mdata['datafield_id'] . "' AND `person_id` = '" . $mdata['person_id'] . "'",
                                                        array('value' => $mdata['value'] . ', ' . $data['value']));
                                                }
                                            }
                                        }
                                    }
                    ...
                    foreach ($entries as $entry) {
                        if (db_get_count('ko_leute_preferred_fields', 'id',
                                "AND `type` = '" . $entry['type'] . "' AND `lid` = '$merged_id'") == 0
                        ) {
                            db_insert_data('ko_leute_preferred_fields',
                                array('type' => $entry['type'], 'lid' => $merged_id, 'field' => $entry['field']));
                        }
                    }
                    ...
                    if (sizeof($relationFields) > 0) {
                        foreach ($relationFields as $field) {
                            ...
                            foreach ($toBeMerged as $merge) {
                                ...
                                db_update_data('ko_leute', "WHERE `id` = '" . $merge['id'] . "'",
                                    array($field => implode(',', $cIDs)));
                            }
                        }
                    }
        ...
        //Show moderations
        if ($showMutations && $access['leute']['MAX'] > 1) {
    ...
    case "multiedit":
        /* Leute-Multiedit */
        if (!$_SESSION['show']) $_SESSION['show'] = 'show_all';
        if ($_SESSION["show"] == "show_all" || $_SESSION["show"] == "show_my_list") {
            ...
            if (sizeof($do_columns) < 1) {
                koNotifier::Instance()->addError(4);
            }
            ...
            foreach ($_POST["chk"] as $c_i => $c) {
                if ($c) {
                    ...
                    if ($access['leute']['ALL'] > 1 || $access['leute'][$edit_id] > 1) {
                        $do_ids[] = $edit_id;
                    }
                }
            }
    ...
    case "submit_multiedit":
        if (empty($_SESSION['show'])) $_SESSION['show'] = 'multiedit';
        if ($_SESSION["show"] == "multiedit") {
            kota_submit_multiedit(2, '', 'lastchange');
        } else {
            if ($_SESSION["show"] == "multiedit_kg") {
                if ($access['kg']['MAX'] < 3) {
                    continue;
                }
    ...
    case 'undelete_person':
        ...
        if (!($access['leute']['ALL'] > 2 || $access['leute'][$del_id] > 2)) {
            continue;
        }

        ko_get_person_by_id($del_id, $del_person);
        if ($del_person['deleted'] != 1) {
            continue;
        }
    ...
    case "submit_mutation":
        ...
        if ($access['leute']['ALL'] < 2 && ($access['leute'][$mod_p['_leute_id']] < 2 || $mod_p['_leute_id'] < 1)) {
            continue;
        }
        ...
        foreach ($_POST["chk_" . $mod_aa_id] as $field => $field_state) {
            ...
            if ($field == "email" && !$new_entry && defined("EXPORT2EZMLM") && EXPORT2EZMLM) {
                if ($old_p["email"] != $data["email"] && check_email($data[$field])) {  //Check for changed valid email
                    foreach (explode(",",
                        $old_p["groups"]) as $group) {  //Check this user's groups for one with an ML assigned
                        $gid = ko_groups_decode($group, "group_id");
                        if ($all_groups[$gid]["ezmlm_list"]) {
                            //Un- and resubscribe
                            ko_ezmlm_unsubscribe($all_groups[$gid]["ezmlm_list"], $all_groups[$gid]["ezmlm_moderator"],
        ...
        if (!$found) {
            continue;
        }  //Keine markierte CheckBox gefunden
        ...
        if ($old_p["famid"] != 0 && $_POST["chk_" . $mod_aa_id]['decouple_from_family']) {
            ...
            if ($num <= 1) {
                db_update_data('ko_leute', "WHERE `famid` = '" . $old_p["famid"] . "'",
                    array('famid' => '0', 'kinder' => '0'));
    ...
    case "submit_gs_ps_aa":  //Person from peoplesearch and add change
        if ($access['leute']['ALL'] < 4 && !($access['leute']['GS'] && $access['groups']['MAX'] > 1)) {
            continue;
        }
        if (!$_POST["_id"]) {
            continue;
        ...
        ko_get_groupsubscriptions($_p, $_id);
        $_p = $_p[$_id];
        ...
        if (isset($_POST['gs_role_' . $_id])) {
            ...
            if ($rid) {
                $save_group_id = $save_group_id . ":" . $rid;
            }
        }
        ...
        if ($do_action == "submit_gs_new_person") {
            ...
            if ($do_action == "submit_gs_ps" || $do_action == "submit_gs_ps_aa") {
        ...
        foreach ($lids as $lid) {
            ...
            foreach (explode(",", $all_groups[$gid]["datafields"]) as $fid) {
                ...
                db_delete_data("ko_groups_datafields_data",
                    "WHERE `datafield_id` = '$fid' AND `person_id` = '$lid' AND `group_id` = '$gid'");
                db_insert_data("ko_groups_datafields_data",
                    array("group_id" => $gid, "person_id" => $lid, "datafield_id" => $fid, "value" => $value));
            }

            //Set default tracking data
            if (in_array('tracking', $MODULES)) {
                $tracking = db_select_data('ko_tracking', "WHERE `filter` REGEXP '^g" . $gid . "[:r0-9]*'", '*', '',
                    'LIMIT 0,1', true);
        ...
        ko_log("groupsubscription",
            $p["vorname"] . " " . $p["nachname"] . ": " . $all_groups[$gid]["name"] . ' - ' . $logdata);
        break;


    case "submit_gs_delete":
        if ($access['leute']['ALL'] < 4) {
            continue;
        }
        if (!$_POST["_id"]) {
            continue;
        ...
        ko_log("del_groupsubscription",
            $_p["vorname"] . " " . $_p["nachname"] . ": " . $_p["_group_id"] . ' - ' . $logdata);
        break;
    ...
    case "leute_action":
        ...
        switch ($_POST["sel_cols"]) {
                ...
                foreach (explode(',', ko_get_userpref($_SESSION['ses_userid'], 'leute_children_columns')) as $col) {
                    ...
                    if (!$ll) {
                        if (in_array(substr($col, 0, 8), array("_father_", "_mother_"))) {
                            $ll = getLL("kota_ko_leute_" . substr($col,
                                        8)) . " " . getLL("leute_children_col" . substr($col, 0, 7));
                        } else {
            ...
            default:
                if (substr($_POST["sel_cols"], 0, 4) == "set_") {
                    ...
                    if (substr($setname, 0, 3) == '@G@') {
                        $set = ko_get_userpref('-1', substr($setname, 3), "leute_itemset");
                    } else {
                        $set = ko_get_userpref($_SESSION["ses_userid"], $setname, "leute_itemset");
                    }
        ...
        if ($_SESSION['show'] == 'geburtstagsliste') {
            if (!in_array('geburtsdatum', $xls_cols)) {
                $xls_cols[] = 'geburtsdatum';
            }
            if (!in_array('alter', $xls_cols)) {
                $xls_cols[] = 'alter';
            }
            if (!in_array('deadline', $xls_cols)) {
                $xls_cols[] = 'deadline';
            }
        ...
        if (substr($_POST["sel_auswahl"], 0,
                4) == "alle" && ($_SESSION["show"] == "show_all" || $_SESSION["show"] == "xls_settings")
        ) {
            ...
            if (($_POST['sel_auswahl'] == 'markierte' || substr($_POST['sel_auswahl'], 0,
                        4) == 'alle') && $_SESSION['show'] == 'geburtstagsliste'
            ) {
                $mode = 'p';
                ...
                if ($_POST['sel_auswahl'] == 'markierte' && isset($_POST['ids'])) {
                    ...
                    foreach (explode(',', $_POST['ids']) as $c_i => $c) {
                        if ($c) {
                            $pids[] = format_userinput($c_i, 'uint');
                        }
                    }
                ...
                if (substr($_POST["sel_auswahl"], 0, 4) == "alle") {
                    $mode = substr($_POST["sel_auswahl"], 4);
                    $pidlist = $_SESSION["my_list"];
                } else {
                    if ($_POST["sel_auswahl"] == "markierte") {
                        $mode = "f";
                        $clear_famid_for_pidlist = true;
                        if (isset($_POST["ids"])) {
                            foreach (explode(',', $_POST["ids"]) as $c) {
                                if ($c) {
                                    $pidlist[] = format_userinput($c, "uint");
                                }
                            }
                        }

                        //TODO: famchk in POST �bergeben
                        if (isset($_POST["famchk"])) {
                            foreach ($_POST["famchk"] as $c_i => $c) {
                                if ($c) {
                                    $famlist[] = format_userinput($c_i, "uint");
                                }
                            }
                        ...
                        if ($_POST["sel_auswahl"] == "markiertef") {
                            $mode = "f";
                            if (isset($_POST["ids"])) {
                                foreach (explode(',', $_POST["ids"]) as $c) {
                                    if ($c) {
                            ...
                            if ($_POST["sel_auswahl"] == "markierteFam2") {
                                $mode = "Fam2";
                                if (isset($_POST["ids"])) {
                                    foreach (explode(',', $_POST["ids"]) as $c) {
                                        if ($c) {
        ...
        if ($clear_famid_for_pidlist) {
            $clear_pidlist = $pidlist;
        }
        //get family member for all selected families
        if (sizeof($famlist) > 0) {
            foreach ($famlist as $famid) {
                $members = db_select_data('ko_leute',
                    ("WHERE `famid` = '$famid' AND `famfunction` IN ('husband', 'wife') AND `deleted` = '0'" . ko_get_leute_hidden_sql()),
                    'id');
        ...
        if ($mode == "Fam2") {
            ...
            foreach ($fam as $famid => $pids) {
                ...
                if (sizeof($pids) == 1) {
                    foreach ($pids as $pid) {
                        $es[$pid]["famid"] = "";
                    }  //And unset this famid, so it will be exported as person
                    ...
                    if (sizeof($pids) > 1) {
                        ...
                        if (in_array("husband", $famroles)) {
                            $keep = "husband";
                        } else {
                            if (in_array("wife", $famroles)) {
                                $keep = "wife";
                            } else {
                                $keep = "";
                            }
        ...
        //Force rectype
        if ($_POST['sel_rectype'] && (in_array($_POST['sel_rectype'],
                    array_keys($RECTYPES)) || $_POST['sel_rectype'] == '_default')
        ) {
        ...
        $data = array();
        foreach ($es as $pid => $p) {
            if (($access['leute']['ALL'] < 1 && $access['leute'][$pid] < 1) || !$pid) {
                continue;
            }
            ...
            if (($mode == "f" && $p["famid"])
                ...
                if ( // ehepaar export
                    ...
                    if (in_array('anrede', $xls_cols)) {
                        ...
                        if ($families[$p['famid']]['famanrede']) {
                            ...
                            foreach ($fam[$p['famid']] as $member_id) {
                                if (!in_array($orig_es[$member_id]['famfunction'], array('husband', 'wife'))) {
                                    $child = true;
                                }
                            }
                            if ($child) {
                                $p['anrede'] = getLL('ko_leute_anrede_family');
                            } else {
                                $p['anrede'] = getLL('ko_leute_anrede_family_no_children');
                            }
                        }
                    ...
                    if (in_array("vorname", $xls_cols)) {
                        ...
                        if (!$families[$p["famid"]]["famanrede"] && !$families[$p["famid"]]["famfirstname"] && $families[$p["famid"]]["famlastname"] && ko_get_userpref($_SESSION['ses_userid'],
                                'leute_force_family_firstname') == 0
                        ) {
                            $p["vorname"] = "";
                        } else {
                            if (ko_get_userpref($_SESSION['ses_userid'], 'leute_force_family_firstname') == 2) {
                                ...
                                $familyMembers = (array)db_select_data('ko_leute',
                                    "WHERE `famid` = '" . $p['famid'] . "' AND `famfunction` IN ('husband', 'wife')",
                                    'id,famfunction,vorname', 'ORDER BY famfunction ASC');
                                $familyMembers = array_merge($familyMembers, (array)db_select_data('ko_leute',
                                    "WHERE `famid` = '" . $p['famid'] . "' AND `famfunction` IN ('child', '')",
                                    'id,famfunction,vorname', 'ORDER BY famfunction DESC, geburtsdatum DESC'));
                                $foundMembers = array();
                                foreach ($familyMembers as $oneMember) {
                                    if (in_array($oneMember['id'], array_keys($orig_es))) {
                                        $foundMembers[] = $oneMember['vorname'];
                                    }
                                }
                                ...
                                if ($families[$p["famid"]]["famfirstname"]) {
                                    ...
                                    $parents = db_select_data("ko_leute",
                                        "WHERE `famid` = '" . $p["famid"] . "' AND `famfunction` IN ('husband', 'wife')",
                                        "famfunction,vorname", "ORDER BY famfunction ASC");
                                    $parent_values = array();
                ...
                if (in_array('email', $xls_cols) || $_POST['id'] == 'email') {
                    if ($families[$p['famid']]['famemail']) {  //Get family email address if set
                        $parent = db_select_data('ko_leute',
                            ("WHERE `famid` = '" . $p['famid'] . "' AND `famfunction` = '" . $families[$p['famid']]['famemail'] . "' AND `deleted` = '0'" . ko_get_leute_hidden_sql()),
                            '*', '', 'LIMIT 0,1', true);
                        ...
                        if ($p['famfunction'] == 'child') {  //if no family email is set but the person is a child, use the email address of one of the parents
                            $parents = db_select_data('ko_leute',
                                ("WHERE `famid` = '" . $p['famid'] . "' AND `famfunction` IN ('husband', 'wife') AND `deleted` = '0'" . ko_get_leute_hidden_sql()),
        ...
        if (!$notifier->hasErrors()) {
            switch ($_POST["id"]) {
                ...
                case 'csv':
                    ...
                    if ($_POST['id'] == 'excel') {
                        $filename = $ko_path . "download/excel/" . getLL("export_filename") . strftime("%d%m%Y_%H%M%S",
                                time()) . ".xlsx";
                        $filename = ko_export_to_xlsx($header, $data, $filename, "kOOL", 'landscape', $wrap, array(),
                            $linebreak_columns);
                    } else {
                        $filename = $ko_path . 'download/excel/' . getLL('export_filename') . strftime('%d%m%Y_%H%M%S',
                                time()) . '.csv';
                    ...
                    //Fileshare-Datei speichern, falls gew�nscht
                    if (ko_module_installed("fileshare",
                            $_SESSION["ses_userid"]) && ko_get_userpref($_SESSION["ses_userid"],
                            "save_files_as_share") == 1
                    ) {
                ...
                case "sms":
                    ...
                    //Leute-cols f�r Excel-Datei auslesen
                    $cols = array(
                        "anrede",
                        "vorname",
                        "nachname",
                    ...
                    if ($login['mobile'] && in_array($login['mobile'], $sender_ids)) {
                        $senders[] = $login['mobile'];
                    }
                    ...
                    foreach ($mobiles as $mobile) {
                        ...
                        if (!in_array($mobile, $sender_ids)) {
                            continue;
                        }
                        if (in_array($mobile, $senders)) {
                            continue;
                        }
                        $senders[] = $mobile;
                    }
                ...
                case "etiketten":
                    //Vorlage-Namen auslesen (f�r Dropdown-Listen)
                    //$vorlagen["values"][] = "";
                    //$vorlagen["output"][] = "";
                    ...
                    foreach ($col as $i => $c) {
                        $free_cols[] = $c;
                        $free_cols[] = ($i < sizeof($col) - 1 ? array(
                            'id' => 'free_' . ($i + 1),
                            'type' => 'empty',
                            'name' => 'free_' . ($i + 1),
                            'show_select' => true
                    ...
                    if ($person != null && $person['nachname'] != null && trim($person['nachname']) != '' && $person['ort'] != null && trim($person['ort']) != '') {
                        ...
                        if (substr($returnAddressLogin, -2) == ', ') {
                            $returnAddressLogin = substr($returnAddressLogin, 0, -2);
                        }
                    } else {
                    ...
                    if (ko_get_setting('info_name') != null && trim(ko_get_setting('info_name')) != '' && ko_get_setting('info_city') != null && trim(ko_get_setting('info_city')) != '') {
                        ...
                        if (substr($returnAddressInfo, -2) == ', ') {
                            $returnAddressInfo = substr($returnAddressInfo, 0, -2);
                        }
                    } else {
                ...
                case 'mailmerge':
                    ...
                    foreach ($mailmerge_signature_ext as $ext) {
                        if (file_exists($ko_path . 'latex/images/signature_' . $_SESSION['ses_userid'] . '.' . $ext)) {
                            $smarty->assign('show_sig_file',
                                $ko_path . 'latex/images/signature_' . $_SESSION['ses_userid'] . '.' . $ext);
                        }
                    }
                    ...
                    //Get letters
                    $rows = db_select_data('ko_mailmerge', 'WHERE `user_id` = \'' . $_SESSION['ses_userid'] . '\'', '*',
                    ...
                    if (sizeof($rows) > 0) {
                        ...
                        foreach ($rows as $letter) {
                            ...
                            $letters[] = strftime($DATETIME['dmy'],
                                    strtotime($letter['crdate'])) . ' ' . ko_html(substr($letter['subject'], 0,
                                    40)) . ' (' . $letter['num_recipients'] . ')';
                        }
                ...
                case "email":
                    ...
                    $ohne_email = "";
                    $row = 0;
                    foreach ($es as $l => $p) {  //Loop �ber alle Leute
                    ...
                    if ($ohne_email != "") {
                        $dateiname = $ko_path . "download/excel/" . getLL("export_filename") . strftime("%d%m%Y_%H%M%S",
                                time()) . ".xlsx";
    ...
    case "submit_sms":
        ...
        if ($_POST['sel_sender'] && in_array($_POST['sel_sender'], $senders)) {  //User selected sener from GUI if set
            ...
            if (sizeof($senders) > 0) {  //Use first (possibly only) entry of senders
                $from = array_shift($senders);
            } else {
                if (!ko_get_logged_in_id()) {  //If no person is assigned just use the login name
        ...
        if ($SMS_PARAMETER['provider'] == 'aspsms') {
            ...
            $msg_type = "SMS_TEXT";
            $ret = send_sms($empfaenger, $text, $from, $climsgid, $msg_type, $success, $done, $problems, $charges,
                $error_message);
            ...
            if ($ret) {
                ...
                if ($problems) {
                    $info_txt .= getLL('info_leute_99c') . substr($problems, 0, -2);
                }
                $info_txt .= getLL('info_leute_99d') . ' ' . $charges;
            } else {
    ...
    case "exportadd_sms_to_mylist":
        ...
        $smarty->assign("tpl_recipients_invalid_ids",
            format_userinput($_POST["hid_recipients_invalid_ids"], "intlist"));
    ...
    case "submit_etiketten":
        ...
        //Etiketten erstellen
        $filename = ko_export_etiketten($_POST['sel_vorlage'], $_POST['txt_start'], $_POST['rd_rahmen'], $labels_data,
            ($_POST['chk_fill_page'] ? $_POST['txt_fill_page'] : 0), $_POST['txt_multiply'], $retChk == 1, $retSel,
            $retTxt);

        //Fileshare-Datei speichern, falls gew�nscht
        if (ko_module_installed("fileshare", $_SESSION["ses_userid"]) && ko_get_userpref($_SESSION["ses_userid"],
                "save_files_as_share") == 1
        ) {
            ko_fileshare_save_file_as_share($_SESSION["ses_userid"], "../" . $filename);  //F�hrendes "../" vorstellen
        }
    ...
    case 'submit_mailmerge':
        ...
        foreach ($data as $p_id => $p) {
            ...
            if ($_POST['rd_salutation'] == 'formal') {
                if ($p['famid'] != 0 && in_array($p['famid'], $fam_ids)) {  //Family
                    ...
                    if (!$p['anrede'] && $p['geschlecht']) {
                        $p['anrede'] = getLL('leute_salutation_' . $p['geschlecht']);
                    }
            ...
            foreach (explode("\n", $text) as $line) {
                ...
                if ($line == '') {
                    ...
                    if (substr($line, 0, 1) == '-') {  //List
                        ...
                        if ($itemize) {  //no more item but still inside a list, so finish the list
                            $itemize = false;
                            $txt .= '\end{itemize}' . "\n" . $line . "\n";
        ...
        if (is_file($ko_path . 'latex/compile/' . $file . '.pdf')) {
            $filename = $ko_path . 'download/pdf/' . getLL('leute_mailmerge_filename') . strftime('%d%m%Y_%H%M%S',
                    time()) . '.pdf';
    ...
    case "submit_email":
        ...
        if ($m["email"] == "") {
            $email = ko_get_setting("info_email");
        } else {
            $email = $m["email"];
        }
        ...
        if (!$notifier->hasErrors()) {
            ...
            ko_log("leute_email", '"' . format_userinput($_POST["txt_betreff"], "text") . '": ' . str_replace(",", ", ",
                    format_userinput($_POST["txt_empfaenger"], "text")) . ", cc: " . str_replace(",", ", ",
                    format_userinput($_POST["txt_cc"], "text")) . ", bcc: " . str_replace(",", ", ",
                    format_userinput($_POST["txt_bcc"],
                        "text")) . ", Text: " . format_userinput($_POST["txt_emailtext"], "text"));
        }
        break;
    ...
    case "set_kg_filter":
        ...
        if ($access['kg']['MAX'] < 1 || $access['leute']['MAX'] < 1 || !$id) {
            continue;
        }
    ...
    case 'kg_xls_export':
        ...
        if ($cols == '_session') {
            ...
            if ($cols == '_all') {
                ...
                if (substr($name, 0, 3) == '@G@') {
                    $preset = ko_get_userpref('-1', substr($name, 3), 'leute_kg_itemset');
                } else {
                    $preset = ko_get_userpref($_SESSION['ses_userid'], $name, 'leute_kg_itemset');
                }
        ...
        if ($use_cols == '') {
            $use_cols = $KOTA['ko_kleingruppen']['_listview_default'];
        }
    ...
    case "set_group_filter":
        ...
        if ($access['groups']['MAX'] < 1 || $access['leute']['MAX'] < 1 || !$id) {
            continue;
        }
        ...
        if (ko_get_userpref($_SESSION['ses_userid'], 'groups_filterlink_add_column')) {
            ...
            if ($_SESSION['show_leute_cols'] == '') {
                $_SESSION['show_leute_cols'] = explode(',',
                    ko_get_userpref($_SESSION['ses_userid'], 'show_leute_cols'));
            }
            ...
            if (!in_array('MODULEgrp' . $id, $_SESSION['show_leute_cols'])) {
                ...
                if (ko_get_userpref($_SESSION['ses_userid'], 'group_shows_datafields') == 1) {
                    foreach ($cols as $col) {
                        if (substr($col, 0, 15) != 'MODULEgrp' . $id) {
                            continue;
                        }
                        if (!in_array($col, $_SESSION['show_leute_cols'])) {
                            $_SESSION['show_leute_cols'][] = $col;
                        }
                    }
                }
    ...
    //Import
    case "import":
        if ($access['leute']['ALL'] > 1 || ($access['leute']['MAX'] > 1 && ko_get_leute_admin_groups($_SESSION['ses_userid'],
                    'all') !== false)
        ) {
        } else {
            continue;
        ...
        if ($_GET["state"]) {
            $_SESSION["import_state"] = format_userinput($_GET["state"], "uint");
        }
        if ($_GET["mode"]) {
            $_SESSION["import_mode"] = format_userinput($_GET["mode"], "alpha");
        }
        ...
        if ($_POST["submit"]) {
            if ($_SESSION["import_state"] == 2) {
                if ($_SESSION["import_mode"] == "vcard") {
                    ...
                    if ($_SESSION["import_mode"] == "csv") {
                        //db columns
                        if ($_POST["sel_dbcols"]) {
                            $_SESSION["import_csv"]["dbcols"] = explode(",",
                                format_userinput($_POST["sel_dbcols"], "alphanumlist"));
                        } else {
    ...
    case "do_import":
        ...
        foreach ($_SESSION["import_data"] as $data) {
            if (sizeof($data) > 0) {
                ...
                //Create LDAP entry
                if ($do_ldap) {
                    ko_ldap_add_person($ldap, $data, $new_id);
                }
        ...
        //HOOK: Plugins erlauben, hier einzugreifen, bevor die Session-Daten gel�scht werden
    ...
    case "submit_leute_version":
        ...
        if ($_POST["date_version"] == "") {
            ...
            //empty date
            if (strtotime($version) == 0) {
                continue;
            }
            //don't allow future dates
            if (strtotime($version) > time()) {
                continue;
            }
    ...
    case "rollback":
        ...
        if (isset($data['dienst']) || isset($data['dienstleiter'])) {
            ...
            foreach (explode(',', $data['dienst']) as $did) {
                ...
                if (!isset($all_teams[(int)$did])) {
                    continue;
                }
                list($gid) = explode(',', $all_teams[(int)$did]['group_id']);
                $full_gid = ko_groups_decode($gid, 'full_gid');
                if ($role_team && false === strpos($full_gid, ':r')) {
                    $full_gid .= ':r' . $role_team;
                }
                $grps[] = $full_gid;
            }
            ...
            foreach (explode(',', $data['dienstleiter']) as $did) {
                ...
                list($gid) = explode(',', $all_teams[(int)$did]['group_id']);
                $full_gid = ko_groups_decode($gid, 'full_gid');
                if ($role_leader && false === strpos($full_gid, ':r')) {
                    $full_gid .= ':r' . $role_leader;
                }
                $grps[] = $full_gid;
            }
        ...
        $changed_groups = array_unique(array_merge(array_diff(explode(',', $data['groups']),
            explode(',', $current['groups'])),
            array_diff(explode(',', $current['groups']), explode(',', $data['groups']))));
    ...
    case 'set_fastfilter':
        ...
        foreach ($fast_filter as $id) {
            if ($_POST['fastfilter' . $id]) {
                $_SESSION['filter'][] = array(
                    $id,
                    array('', str_replace('*', '.*', format_userinput($_POST['fastfilter' . $id], 'text'))),
                    0
                );
            }
        }
    ...
    case 'submit_leute_settings':
        ...
        ko_save_userpref($_SESSION['ses_userid'], 'show_limit_leute',
            format_userinput($_POST['txt_limit_leute'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'show_limit_kg', format_userinput($_POST['txt_limit_kg'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'leute_sort_birthdays',
            format_userinput($_POST['sel_leute_sort_birthdays'], 'alpha'));
        ko_save_userpref($_SESSION['ses_userid'], 'hide_leute_filter',
            format_userinput($_POST['sel_hide_filter'], 'intlist'));
        ko_save_userpref($_SESSION['ses_userid'], 'leute_fast_filter',
            format_userinput($_POST['sel_fast_filter'], 'intlist'));
        ko_save_userpref($_SESSION['ses_userid'], 'leute_children_columns',
            format_userinput($_POST['sel_children_columns'], 'alphanumlist'));
        ko_save_userpref($_SESSION['ses_userid'], 'leute_kg_as_cols',
            format_userinput($_POST['sel_kg_as_cols'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'show_passed_groups',
            format_userinput($_POST['chk_show_passed_groups'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'group_shows_datafields',
            format_userinput($_POST['chk_group_shows_datafields'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'leute_fam_checkbox',
            format_userinput($_POST['chk_fam_checkbox'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'leute_carddav_filter',
            format_userinput($_POST['sel_carddav_filter'], 'uint'));
        ...
        if ($birthday_filter == '') {
            ...
            if ($birthday_filter == -1) {
                ...
                if ($filter['value']) {
                    ko_save_userpref($_SESSION['ses_userid'], 'birthday_filter', serialize($filter));
                }
            }
        }
        ...
        if ($access['leute']['ALL'] > 2) {
            ...
            if ($v == 0 || $v == 1 || $v == 2) {
                ko_set_setting('leute_hidden_mode', $v);
            }
            ko_set_setting('leute_real_delete', format_userinput($_POST['chk_real_delete'], 'uint'));
            ko_set_setting('leute_no_delete_columns',
                format_userinput($_POST['sel_no_delete_columns'], 'alphanumlist'));
            ko_set_setting('leute_assign_global_notification',
                format_userinput($_POST['txt_assign_global_notification'], 'email', false, 0, array(), ','));
        }
    ...
    case 'global_assign':
        ...
        $address_text = '';
        if ($address['firm']) {
            $address_text .= $address['firm'] . "\n";
        }
        if ($address['vorname'] || $address['nachname']) {
            $address_text .= $address['vorname'] . ' ' . $address['nachname'] . "\n";
        }
        if ($address['adresse']) {
            $address_text .= $address['adresse'] . "\n";
        }
        if ($address['adresse_zusatz']) {
            $address_text .= $address['adresse_zusatz'] . "\n";
        }
        if ($address['plz'] || $address['ort']) {
            $address_text .= $address['plz'] . ' ' . $address['ort'] . "\n";
        }
...
if (sizeof($access['leute']) > 3 && in_array($do_action, array(
        'submit_neue_person',
        'submit_edit_person',
        'submit_als_neue_person',
        'submit_multiedit',
        'submit_gs',
        'submit_gs_aa',
        'submit_gs_new_person',
        'submit_gs_ps',
        'submit_gs_ps_aa',
        'rollback',
        'global_assign'
...
if ($_SESSION["show_leute_chart"] == "") {
    ...
    if ($userpref) {
        ...
        foreach ($_SESSION["show_leute_chart"] as $i => $chart) {
            if (!in_array($chart, $LEUTE_CHART_TYPES)) {
                unset($_SESSION["show_leute_chart"][$i]);
            }
        }
    } else {
...
if (!$_SESSION["show_limit"]) {
    $_SESSION["show_limit"] = ko_get_setting("show_limit_leute");
}
...
if (!$_SESSION["show_kg_limit"]) {
    $_SESSION["show_kg_limit"] = ko_get_setting("show_limit_kg");
}
...
if (!isset($_SESSION["my_list"])) {
    ...
    foreach ($_SESSION['my_list'] as $k => $v) {
        if (!$v) {
            unset($_SESSION['my_list'][$k]);
        }
    }
}
if (!$_SESSION["filter"]["link"]) {
...
echo \Peregrinus\Flockr\Legacy\Services\LayoutService::pageHeader(
    ...
    $js_files = [
        ...
        FLOCKR_baseUrl. 'inc/selectmenu.js',
        FLOCKR_baseUrl. 'inc/ZeroClipboard.min.js'
    ];
    ...
    if ($_SESSION["show"] == "neue_person" || $_SESSION["show"] == "edit_person") {
        ...
        //Beim Filter die vergangenen Gruppen gem�ss Einstellung zeigen
...
ko_get_submenu_code("leute", "left");
ko_get_submenu_code("leute", "right");
        ...
        <form action="index.php" method="post" name="formular" enctype="multipart/form-data">
                ...
                <div name="main_content" id="main_content">
                    ...
                    switch ($_SESSION["show"]) {
                        ...
                        case "edit_person":
                            ko_formular_leute('edit', format_userinput($_POST['id'], 'uint'),
                                ($hide_save_as_new === true ? false : true));
                            break;
                        ...
                        case "mailmerge":
                            $smarty->assign('label_title',
                                getLL('leute_mailmerge_title') . ' (' . sizeof($_SESSION['mailmerge_data']) . ')');
                        ...
                        case "multiedit":
                            ko_multiedit_formular("ko_leute", $do_columns, $do_ids, $order,
                                array("cancel" => "show_all"));
                            break;

                        case "multiedit_kg":
                            ko_multiedit_formular("ko_kleingruppen", $do_columns, $do_ids, $order,
                                array("cancel" => "list_kg"));
                            break;

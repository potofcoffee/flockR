...
switch ($do_action) {
    ...
    case "list_groups":
        if (isset($_GET["gid"])) {
            if ($_GET["gid"] == "NULL") {
                ...
                $_SESSION["show_gid"] = format_userinput($_GET["gid"], "uint");
            }
        } else {
            if (!isset($_SESSION["show_gid"]) || ko_get_userpref($_SESSION["ses_userid"], "groups_show_top") == 1) {
                $_SESSION["show_gid"] = "NULL";
            }
        }
    ...
    case "exportxls":
        ...
        if ($gs_pid) {
            $headerrow[] = getLL('groups_listheader_event');
        }
        ...
        foreach ($groups as $grp) {
            //Display hierarchy
            for ($i = 0; $i < sizeof($grp['_motherline']); $i++) {
                $data[$rowcounter][] = "";
            }
            //Name
            ...
            if ($gs_pid) {
                ...
                if ($event['startdatum']) {
                    if ($event['startdatum'] != $event['enddatum']) {
                        $data[$rowcounter][] = strftime($DATETIME['dmY'],
                                strtotime($event['startdatum'])) . ' - ' . strftime($DATETIME['dmY'],
                                strtotime($event['enddatum']));
                    } else {
            ...
            //Start and stop
            $data[$rowcounter][] = $grp['start'] != '0000-00-00' ? strftime($DATETIME['dmY'],
                strtotime($grp['start'])) : '-';
            $data[$rowcounter][] = $grp['stop'] != '0000-00-00' ? strftime($DATETIME['dmY'],
                strtotime($grp['stop'])) : '-';

            //Total number of assigned addresses
            $num = db_get_count('ko_leute', 'id',
                "AND `deleted` = '0' AND `hidden` = '0' AND `groups` REGEXP 'g" . $grp['id'] . "'");
            $data[$rowcounter][] = $num;
            ...
            if ($grp["type"]) {  //Dummy group
                ...
                foreach (explode(",", $grp["roles"]) as $rid) {
                    ...
                    $num = db_get_count("ko_leute", "id",
                        "AND `deleted` = '0' AND `hidden` = '0' AND `groups` REGEXP 'g" . $grp["id"] . ":r" . $rid . "'");
    ...
    case "submit_new_group":
        ...
        if ($data['pid'] == 'NULL' && $access['groups']['ALL'] < 3) {
            continue;
        }
        if ($data['pid'] != 'NULL' && $access['groups']['ALL'] < 3 && $access['groups'][$data['pid']] < 3) {
            continue;
        }
        ...
        //Access check for pid
        if ($data['linked_group'] == 'NULL' && $access['groups']['ALL'] < 3) {
            continue;
        }
        if ($data['linked_group'] != 'NULL' && $access['groups']['ALL'] < 3 && $access['groups'][$data['linked_group']] < 3) {
            continue;
        }
    ...
    case "edit_group":
        ...
        if (!$edit_id && isset($_GET['id']) && $_GET['action'] == 'edit_group') {
            $edit_id = format_userinput($_GET['id'], 'uint');
        }
    ...
    case "submit_edit_group":
        ...
        if (isset($_POST["sel_rights_view"])) {  //Only handle rights, if set in form
            ...
            foreach ($rv as $i => $v) {
                if (in_array($v, $rn) || in_array($v, $re) || in_array($v, $rd)) {
                    unset($rv[$i]);
                }
            }
            foreach ($rn as $i => $v) {
                if (in_array($v, $re) || in_array($v, $rd)) {
                    unset($rn[$i]);
                }
            }
            foreach ($re as $i => $v) {
        ...
        foreach (explode(",", $old_group["datafields"]) as $df) {
            if (in_array($df, $new_df)) {
                continue;
            } else {
                db_delete_data("ko_groups_datafields_data", "WHERE `group_id` = '$id' AND `datafield_id` = '$df'");
            }
        }
        //Initial export to ezmlm if given
        if ($_POST["chk_ezmlm_export"]) {
            ko_export_group_to_ezmlm($id);
        }
    ...
    case "edit_role":
        ...
        foreach ($groups as $group) {
            if ($access['groups']['ALL'] < 3 && $access['groups'][$group['id']] < 3) {
                $do_edit = false;
            }
        }
        if ($do_edit) {
    ...
    case 'delete_group':
        ...
        if (($access['groups']['ALL'] > 3 || $access['groups'][$del_id] > 3) && db_get_count('ko_groups', 'id',
                "AND `pid` = '$del_id'") == 0
        ) {
            //Delete group itself
    ...
    case "delete_role":
        ...
        $do_del = true;
        foreach ($groups as $group) {
            if ($access['groups']['ALL'] < 4 && $access['groups'][$group['id']] < 4) {
                $do_del = false;
            }
        }
        if ($do_del) {
    ...
    case "do_delete_role":
        ...
        if ($do_del) {
            ...
            //Rollen in alle Gruppen l�schen, in denen sie vorkommt
            $gruppen = db_select_data("ko_groups", "WHERE `roles` REGEXP '$del_id'");
            foreach ($gruppen as $gruppe) {
                $roles = explode(",", $gruppe["roles"]);
                foreach ($roles as $r_i => $role) {
                    if ($role == $del_id) {
                        unset($roles[$r_i]);
                    }
                }
                db_update_data("ko_groups", "WHERE `id` = '" . $gruppe["id"] . "'",
    ...
    case "delete_datafield":
        ...
        //Pr�fen, ob Datenfeld noch irgendwo verwendet wird, dann kann man es nicht l�schen
    ...
    case "submit_edit_login_rights":
        if ($access['groups']['ALL'] < 3) {
            continue;
        }
        $login_id = format_userinput($_POST["id"], "uint");
        if (!$login_id || ($login_id == ko_get_root_id() && $_SESSION["ses_userid"] != ko_get_root_id())) {
            continue;
        }
        ...
        for ($i = 3; $i > 0; $i--) {
            if (isset($_POST["sel_rights_" . $mode[$i]])) {
                ...
                $old = explode(",",
                    format_userinput($_POST["old_sel_rights_" . $mode[$i]], "intlist", false, 0, array(), ":"));
                $new = explode(",",
                    format_userinput($_POST["sel_rights_" . $mode[$i]], "intlist", false, 0, array(), ":"));
                ...
                foreach ($deleted as $id) {
                    ...
                    db_update_data("ko_groups", "WHERE `id` = '$id'",
    ...
    case 'submit_groups_settings':
        ko_save_userpref($_SESSION['ses_userid'], 'show_limit_groups',
            format_userinput($_POST['txt_limit_groups'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'default_view_groups',
            format_userinput($_POST['sel_default_view'], 'js'));
        ko_save_userpref($_SESSION['ses_userid'], 'show_passed_groups',
            format_userinput($_POST['chk_show_passed_groups'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'groups_filterlink_add_column',
            format_userinput($_POST['chk_groups_filterlink_add_column'], 'uint'));
        ko_save_userpref($_SESSION['ses_userid'], 'groups_show_top',
            format_userinput($_POST['chk_groups_show_top'], 'uint'));
    ...
    case "submit_multiedit":
        if ($_SESSION["show"] == "multiedit") {
            ...
            if ($_SESSION["show"] == "multiedit_roles") {
                if ($access['groups']['MAX'] < 3) {
                    continue;
                }
                kota_submit_multiedit(3);
    ...
    case 'export_pdf':
        ...
        if ($layout['data'] != '' && substr($layout['data'], 0, 4) == 'FCN:' && function_exists(substr($layout['data'],
                4))
        ) {
...
if (!$_SESSION["show_limit"]) {
    $_SESSION["show_limit"] = ko_get_setting("show_limit_groups");
}
if (!isset($_SESSION['groups_show_hidden_datafields'])) {
    $_SESSION['groups_show_hidden_datafields'] = ko_get_userpref($_SESSION['ses_userid'],
        'groups_show_hidden_datafields');
}
...
print ko_include_css();
    ...
    $js_files = [
        ...
        FLOCKR_baseUrl. 'lib/datepicker/bootstrap-datepicker3.min.css',
        FLOCKR_baseUrl. 'inc/kOOL.js'
    ];
    if ($_SESSION['show'] == 'edit_login_rights') {
        $js_files[] = FLOCKR_baseUrl . 'inc/selectmenu.js';
    }
    ...
    //Bei der Bearbeitung von Login-Rechten Ajax einbinden und alles f�r die drei selectmenus
    if ($_SESSION['show'] == 'edit_login_rights') {
...
ko_get_submenu_code("groups", "left");
ko_get_submenu_code("groups", "right");
        ...
        <form action="index.php" method="post" name="formular">  <!-- Hauptformular -->
            ...
            <div name="main_content" id="main_content">
                ...
                switch ($_SESSION["show"]) {
                    ...
                    case "multiedit":
                        ko_multiedit_formular("ko_groups", $do_columns, $do_ids, $order,
                            array("cancel" => "list_groups"));
                        break;

                    case 'multiedit_roles':
                        ko_multiedit_formular('ko_grouproles', $do_columns, $do_ids, $order,
                            array('cancel' => 'list_roles'));
                        break;

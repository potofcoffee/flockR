    ...
    <form action="index.php?action=submit_aa" method="post">
        ...
        {if $tpl_aa_show == "name"}
            <div class="form-group">
                <label for="txt_fm_aa_nachname">{$label_name}</label>:
                <input class="form-control" type="text" name="txt_fm_aa_nachname" size="14"/>
            </div>
            <div class="form-group">
                <label for="txt_fm_aa_vorname">{$label_firstname}</label>:
                <input class="form-control" type="text" name="txt_fm_aa_vorname" size="14"/>
            </div>
            <div class="form-group">
                <input class="btn btn-primary" type="submit" name="submit_fm_aa" value="{$label_ok}"/>
            </div>
        {/if}
        ...
        {if $tpl_aa_show == "fields"}
            ...
            <table border="0">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {if $setting.type == "text"}
                                <input type="text" size="20" name="{$setting.name}"
                                       value="{$setting.value}" {$setting.params} />
                            {elseif $setting.type == "select"}
                                <select name="{$setting.name}" {$setting.params}>
                                    {foreach from=$setting.values item=v key=k}
                                        <option value="{$v}"
                                                {if $v == $setting.value}selected="selected"{/if}>{if $setting.descs.$k}{$setting.descs.$k}{else}{$setting.descs.$v}{/if}</option>
                                    {/foreach}
                                </select>
                            {/if}
                ...
                <tr>
                    <td align="right"><b>{$label_comment}:</b></td>
                    <td><textarea name="txt_bemerkung" cols="40" rows="5"></textarea></td>
                </tr>

...
<div align="right" style="float:right;" class="rota-header">
        ...
        <div id="export_button">
            <div id="export_flyout">
                {foreach from=$exports item=export}
                    <a class="export_item"
                       href="javascript:sendReq('../rota/inc/ajax.php', 'action,mode,sesid', 'export,{$export.mode},{$sesid}', show_box);">
    ...
    <img src="{$ko_path}/images/decrease.png" border="0" alt="-" style="cursor: pointer;"
         onclick="sendReq('../rota/inc/ajax.php', 'action,timespan,sesid', 'settimespan,{$stats.prevts},{$sesid}', do_element);"
         title="{$label_decrease_timespan}"/>
    <select name="sel_timespan" size="0"
            onchange="sendReq('../rota/inc/ajax.php', 'action,timespan,sesid', 'settimespan,'+this.options[this.selectedIndex].value+',{$sesid}', do_element);"
    ...
    <img src="{$ko_path}/images/increase.png" border="0" alt="+" style="cursor: pointer;"
         onclick="sendReq('../rota/inc/ajax.php', 'action,timespan,sesid', 'settimespan,{$stats.nextts},{$sesid}', do_element);"
         title="{$label_increase_timespan}"/>
    ...
    <a href="javascript:sendReq('../rota/inc/ajax.php', 'action,sesid', 'timeplus,{$sesid}', do_element);">
        <img src="{$ko_path}images/icon_arrow_right.png" border="0" alt="{$label_list_next}"
             title="{$label_list_next}"/>
    </a>

    <input name="rota_dateselect"
           onchange="sendReq('../rota/inc/ajax.php', 'action,date,sesid', 'settime,'+this.value+',{$sesid}', do_element);"
           size="1" style="display: none;" id="f-calendar-field-1" type="text"/>
    <a href="#" id="f-calendar-trigger-1"><img align="middle" border="0" class="jsdate-image"
                                               src="../images/calendar.gif" alt=""/></a>
    ...
    <a href="javascript:sendReq('../rota/inc/ajax.php', 'action,sesid', '{$action_date_future},{$sesid}', do_element);">
        <img src="{$ko_path}images/{$icon_date_future}" border="0" alt="{$label_date_future}"
             title="{$label_date_future}"/>
    </a>
    ...
    {foreach from=$weeks item=week}
        <div class="rota-event" name="rota_week_{$week.id}">
            <div class="rota-week-header">
                <div class="rota-header-block">
                    ...
                    {elseif $access_status}
                        <a href="javascript:sendReq('../rota/inc/ajax.php', 'action,id,status,sesid', 'setweekstatus,{$week.id},1,{$sesid}', do_element);">
                            <img src="{$ko_path}images/statusod.png"
                                 onmouseover="this.src='{$ko_path}images/statuso.png';"
                                 onmouseout="this.src='{$ko_path}images/statusod.png';" alt="O" border="0"
                                 title="{$label_status_w_open}"/>
                        </a>
                    {/if}
                    ...
                    {elseif $access_status}
                        <a href="javascript:sendReq('../rota/inc/ajax.php', 'action,id,status,sesid', 'setweekstatus,{$week.id},2,{$sesid}', do_element);">
                            <img src="{$ko_path}images/statuscd.png"
                                 onmouseover="this.src='{$ko_path}images/statusc.png';"
                                 onmouseout="this.src='{$ko_path}images/statuscd.png';" alt="X" border="0"
                                 title="{$label_status_w_close}"/>
                        </a>
                    {/if}
                </div>
                ...
                {/if}
                <div name="rota_stats_{$week.id}" class="rota-header-block">
                    <div class="{$class}">{$week._stats.done}/{$week._stats.total}</div>
                </div>
...
<div class="panel-group" id="accRota">
    {foreach from=$events item=event}
        <a id="c{$event.id}"></a>
        <div class="panel-heading rota-event-header" style="border-bottom: solid 3px #{$event.eventgruppen_farbe};">
            <div class="rota-header-block">
                ...
                {elseif $access_status}
                    <a href="javascript:sendReq('../rota/inc/ajax.php', 'action,id,status,sesid', 'seteventstatus,{$event.id},1,{$sesid}', do_element);">
                        <img src="{$ko_path}images/statusod.png" onmouseover="this.src='{$ko_path}images/statuso.png';"
                             onmouseout="this.src='{$ko_path}images/statusod.png';" alt="O" border="0"
                             title="{$label_status_e_open}"/>
                    </a>
                {/if}
                ...
                {elseif $access_status}
                    <a href="javascript:sendReq('../rota/inc/ajax.php', 'action,id,status,sesid', 'seteventstatus,{$event.id},2,{$sesid}', do_element);">
                        <img src="{$ko_path}images/statuscd.png" onmouseover="this.src='{$ko_path}images/statusc.png';"
                             onmouseout="this.src='{$ko_path}images/statuscd.png';" alt="X" border="0"
                             title="{$label_status_e_close}"/>
                    </a>
                {/if}
            </div>
            ...
            {else}
                {assign var="class" value="rota-stats-half"}
            {/if}
            <div name="rota_stats_{$event.id}" class="rota-header-block">
                <div class="{$class}">{$event._stats.done}/{$event._stats.total}</div>
            </div>
            ...
            <div class="rota-event-header-title">
                <a data-toggle="collapse" data-parent="#accRota" href="#panelEvent{$event.id}">
                    {$event._date}
                    &nbsp;
        ...
        <div id="panelEvent{$event.id}" class="panel-collapse collapse">

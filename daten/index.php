...
switch ($do_action) {
    ...
    case 'delete_termin':
        ...
        if ($access['daten'][$event['eventgruppen_id']] < 2) {
            continue;
        }
    ...
    case "delete_gruppe":
        ...
        $log_message = $del_eventgruppe["name"] . ": " . substr($del_eventgruppe["startzeit"], 0,
                -3) . "-" . substr($del_eventgruppe["endzeit"], 0, -3);
        ...
        if (in_array('rota', $MODULES)) {
            ...
            $teams = db_select_data('ko_rota_teams', "WHERE `eg_id` REGEXP '(^|,)$del_id(,|$)'", '*',
                'ORDER BY name ASC');
            if (sizeof($teams) > 0) {
    ...
    case "submit_daten_settings":
        ...
        ko_save_userpref($_SESSION['ses_userid'], 'show_limit_daten',
            format_userinput($_POST['txt_limit_daten'], 'uint'));
        ...
        ko_save_userpref($_SESSION['ses_userid'], 'daten_monthly_title',
            format_userinput($_POST['sel_monthly_title'], 'js', false));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_title_length',
            format_userinput($_POST['txt_title_length'], 'uint', false));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_pdf_show_time',
            format_userinput($_POST['sel_pdf_show_time'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_pdf_use_shortname',
            format_userinput($_POST['sel_pdf_use_shortname'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_export_show_legend',
            format_userinput($_POST['sel_export_show_legend'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_pdf_week_start',
            format_userinput($_POST['sel_pdf_week_start'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_pdf_week_length',
            format_userinput($_POST['sel_pdf_week_length'], 'uint', false, 2));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_mark_sunday',
            format_userinput($_POST['sel_mark_sunday'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_no_cals_in_itemlist',
            format_userinput($_POST['sel_no_cals_in_itemlist'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'show_birthdays',
            format_userinput($_POST['sel_show_birthdays'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_show_res_in_tooltip',
            format_userinput($_POST['sel_show_res_in_tooltip'], 'uint', false, 1));
        ko_save_userpref($_SESSION['ses_userid'], 'daten_rooms_only_future',
            format_userinput($_POST['chk_daten_rooms_only_future'], 'uint', false, 1));
        if ($_SESSION['ses_userid'] != ko_get_guest_id()) {
            ko_save_userpref($_SESSION['ses_userid'], 'daten_ical_deadline',
                format_userinput($_POST['sel_ical_deadline'], 'int', false));
            if ($access['daten']['MAX'] > 3) {
                ko_save_userpref($_SESSION['ses_userid'], 'do_mod_email_for_edit_daten',
                    format_userinput($_POST['sel_do_mod_email_for_edit_daten'], 'uint', false, 1));
            }
        }
        ko_save_userpref($_SESSION['ses_userid'], 'daten_ical_description_fields',
            format_userinput($_POST['sel_ical_description_fields'], 'alphanumlist'));
        ...
        if ($access['daten']['MAX'] > 3) {
            if (in_array('groups', $MODULES)) {
                ...
                ko_set_setting('daten_gs_available_roles',
                    format_userinput($_POST['sel_gs_available_roles'], 'alphanumlist'));
            }
            ko_set_setting('daten_show_mod_to_all', format_userinput($_POST['sel_show_mod_to_all'], 'uint'));
            ko_set_setting('daten_mod_exclude_fields',
                format_userinput($_POST['sel_mod_exclude_fields'], 'alphanumlist'));
            ...
            if ($_SESSION['ses_userid'] == ko_get_root_id()) {
                ko_set_setting('activate_event_program',
                    format_userinput($_POST['sel_activate_event_program'], 'uint'));
            }
        }


        $_SESSION['show'] = ($_SESSION['show_back'] && in_array($_SESSION['show_back'],
                array_keys($DISABLE_SM['daten']))) ? $_SESSION['show_back'] : 'daten_settings';
        break;
    ...
    case "neuer_termin":
        ...
        if ($new_date_start != $new_date_end) {
            kota_assign_values("ko_event", array("enddatum" => $new_date_end));
        }

        $new_event_data = array("start_time" => $new_time_start, "end_time" => $new_time_end);

        if ($_SESSION['show'] != 'neuer_termin') {
            $_SESSION['show_back'] = $_SESSION['show'];
        }
    ...
    case 'submit_as_new_event':
        ...
        switch ($_POST["rd_wiederholung"]) {
            case "taeglich":
                $inc = format_userinput($_POST["txt_repeat_tag"], "uint");
                break;
            case "woechentlich":
                $inc = format_userinput($_POST["txt_repeat_woche"], "uint");
                break;
            case "monatlich1":
                $inc = format_userinput($_POST["sel_monat1_nr"],
                        "uint") . "@" . format_userinput($_POST["sel_monat1_tag"], "uint");
                break;
            case "monatlich2":
                $inc = format_userinput($_POST["txt_repeat_monat2"], "uint");
                break;
        }
        ...
        for ($i = 0; $i < sizeof($repeat); $i += 2) {
            ...
            $data['res_startdatum'] = date('Y-m-d',
                ($_POST['res_startdatum_delta'] * 24 * 3600 + strtotime($data["startdatum"])));
            $data['res_enddatum'] = date('Y-m-d',
                ($_POST['res_enddatum_delta'] * 24 * 3600 + strtotime($data["enddatum"])));
                ...
                if ($do_action == 'submit_as_new_event') {
                    ...
                    foreach ($event_data as $savedEvent) {
                        ...
                        foreach ($fileFields as $fileField) {
                            if ($savedEvent[$fileField] != '') {
                                continue;
                            }
                            if (array_key_exists($fileField . '_DELETE', $savedEvent)) {
                                continue;
                            }
        ...
        $_SESSION['show'] = ($_SESSION['show_back'] && in_array($_SESSION['show_back'],
                array_keys($DISABLE_SM['daten']))) ? $_SESSION['show_back'] : 'calendar';
        break;
    ...
    case 'submit_new_ical':
        ...
        if ($id) {
            ko_daten_import_ical($id);
        }

        $_SESSION['show'] = ($_SESSION['show_back'] && in_array($_SESSION['show_back'],
                array_keys($DISABLE_SM['daten']))) ? $_SESSION['show_back'] : 'all_groups';
        break;
    ...
    case "multiedit":
        if ($_SESSION["show"] == "all_events") {
            ...
            foreach ($_POST["chk"] as $c_i => $c) {
                ...
                $event = db_select_data('ko_event AS e LEFT JOIN ko_eventgruppen AS g ON e.eventgruppen_id = g.id',
                    "WHERE e.id = '$edit_id'",
                    'e.id AS id, e.eventgruppen_id AS eventgruppen_id, g.moderation AS moderation, e.import_id AS import_id',
                    '', '', true);
                ...
                if (($event['moderation'] > 0 && $access['daten'][$event['eventgruppen_id']] > 2) || ($event['moderation'] == 0 && $access['daten'][$event['eventgruppen_id']] > 1)) {
                    $do_ids[] = $edit_id;
                }
            }
            if (sizeof($do_ids) < 1) {
    ...
    case "submit_multiedit":
        ...
        $_SESSION['show'] = ($_SESSION['show_back'] && in_array($_SESSION['show_back'],
                array_keys($DISABLE_SM['daten']))) ? $_SESSION['show_back'] : 'all_events';
        break;
    ...
    case "daten_mod_new_approve_multi":
        ...
        if ($do_action == "daten_mod_new_approve") {
            ...
            if ($do_action == "daten_mod_new_approve_multi") {
                foreach ($_POST["chk"] as $c_i => $c) {
                    if ($c) {
        ...
        foreach ($ids as $id) {
            ...
            if (!$new_event_data["eventgruppen_id"] || $access['daten'][$new_event_data["eventgruppen_id"]] < 4) {
                continue;
            }
        ...
        if ($notification) {
            ...
            $smarty->assign('txt_betreff',
                ('[kOOL] ' . (sizeof($ids) > 1 ? getLL('daten_emails_mod_confirm_subject') : getLL('daten_email_mod_confirm_subject'))));

            $smarty->assign('txt_emailtext',
                ((sizeof($ids) > 1 ? getLL('daten_emails_mod_confirm_text') : getLL('daten_email_mod_confirm_text')) . "\n\n" . ko_html($event_text)));
    ...
    case "daten_mod_delete_multi":
        ...
        if ($do_action == "daten_mod_delete") {
            ...
            if ($do_action == "daten_mod_delete_multi") {
                foreach ($_POST["chk"] as $c_i => $c) {
                    if ($c) {
                        $ids[] = format_userinput($c_i, "uint");
                    }
        ...
        foreach ($ids as $id) {
            ...
            if (!$mod_event["eventgruppen_id"]) {
                continue;
            }
            if ($access['daten'][$mod_event["eventgruppen_id"]] < 4 && $mod_event["_user_id"] != $_SESSION["ses_userid"]) {
                continue;
            }
            ...
            if ($notification) {
                $person = db_select_data("ko_admin AS a LEFT JOIN ko_leute AS l ON a.leute_id = l.id",
                    "WHERE a.id = '" . $mod_event["_user_id"] . "'", "a.id AS id,l.email AS email", "", "", true);
                if ($person["email"]) {
                    $email_rec[] = $person["email"];
                }
        ...
        if ($notification) {
            ...
            $smarty->assign("tpl_show_send", true);
            $smarty->assign('txt_betreff',
                ('[kOOL] ' . (sizeof($ids) > 1 ? getLL('daten_emails_mod_delete_subject') : getLL('daten_email_mod_delete_subject'))));

            $smarty->assign('txt_emailtext',
                ((sizeof($ids) > 1 ? getLL('daten_emails_mod_delete_text') : getLL('daten_email_mod_delete_text')) . "\n\n" . ko_html($event_text)));
    ...
    case "daten_mod_delete_approve_multi":
        ...
        if ($do_action == "daten_mod_delete_approve") {
            ...
            if ($do_action == "daten_mod_delete_approve_multi") {
                foreach ($_POST["chk"] as $c_i => $c) {
                    if ($c) {
                        $ids[] = format_userinput($c_i, "uint");
                    }
        ...
        foreach ($ids as $id) {
            ...
            $mod_event = db_select_data("ko_event_mod", "WHERE `id` = '$id'", "*", "", "", true);
            if (!$mod_event["eventgruppen_id"] || $access['daten'][$mod_event["eventgruppen_id"]] < 4 || !$mod_event["_event_id"]) {
                continue;
            }
        ...
        if ($notification) {
            ...
            $smarty->assign("txt_emailtext",
                (getLL("daten_email_mod_delete_confirm_text") . "\n\n" . ko_html($event_text)));
    ...
    case "daten_mod_edit_approve_multi":
        ...
        if ($do_action == "daten_mod_edit_approve") {
            ...
            if ($do_action == "daten_mod_edit_approve_multi") {
                foreach ($_POST["chk"] as $c_i => $c) {
                    if ($c) {
                        $ids[] = format_userinput($c_i, "uint");
                    }
        ...
        if ($notification) {
            ...
            $smarty->assign("txt_emailtext",
                (getLL("daten_email_mod_edit_confirm_text") . "\n\n" . ko_html($event_text)));
    ...
    case 'submit_email':
        ...
        if (!$notifier->hasErrors()) {
            ...
            ko_log('event_mod_email', '"' . format_userinput($_POST['txt_betreff'],
                    'text') . '": ' . format_userinput($_POST['txt_empfaenger'],
    ...
    case 'edit_reminder':
        ...
        if (!ko_get_reminder_access($editId)) {
            continue;
        }
    ...
    case 'submit_edit_event_reminder':
        ...
        if ($do_action == 'submit_edit_event_reminder') {
            $_POST['kota_type'] = 1;
        }
    ...
    case 'export_xls_daten':
        ...
        if ($cols == '_session') {
            ...
            if (substr($name, 0, 3) == '@G@') {
                $preset = ko_get_userpref('-1', substr($name, 3), 'ko_event_colitemset');
            } else {
                $preset = ko_get_userpref($_SESSION['ses_userid'], $name, 'ko_event_colitemset');
            }
            $value = $preset[0]['value'];
        }
        //Fallback to default columns
        if ($value == '') {
            $value = implode(',', $KOTA['ko_event']['_listview_default']);
        }
    ...
    case 'export_pdf':
        ...
        switch ($mode) {
            ...
            case 'm':
                ...
                $filename = basename(ko_daten_export_months(1, date('m', strtotime($start)),
            ...
            default:
                if (substr($mode, 0, 6) == 'preset') {
                    ...
                    $preset = db_select_data('ko_pdf_layout', "WHERE `id` = '$id' AND `type` = 'daten'", '*', '', '',
                        true);
...
if (!$_SESSION['show_limit']) {
    $_SESSION['show_limit'] = ko_get_setting('show_limit_daten');
}
...
if (in_array($_SESSION['show'],
    array('neue_gruppe', 'edit_gruppe', 'new_googlecal', 'edit_googlecal', 'new_ical'))) {
    print '<script language="javaScript" type="text/javascript">ko_path = \'' . $ko_path . '\'</script>';
    $js_files[] = FLOCKR_baseUrl. 'inc/ColorPicker2.js';
}
...
$js_files[] = FLOCKR_baseUrl. 'inc/ckeditor/ckeditor.js';
$js_files[] = FLOCKR_baseUrl. 'inc/ckeditor/adapters/jquery.js';
...
if (in_array($_SESSION["show"], array("neuer_termin"))) {
    include("inc/js-seleventgroup.inc");
}
...
echo \Peregrinus\Flockr\Legacy\Services\LayoutService::renderSnippet('openSidebar', []);
    ...
    ko_get_submenu_code("daten", "left");
    ko_get_submenu_code("daten", "right");
        ...
        <form action="index.php" method="post" name="formular" enctype="multipart/form-data">
            ...
            <div name="main_content" id="main_content">
                ...
                switch ($_SESSION["show"]) {
                    ...
                    case "multiedit":
                        ko_multiedit_formular("ko_event", $do_columns, $do_ids, $order,
                            array("cancel" => "all_events"));
                        break;

                    case "multiedit_tg":
                        ko_multiedit_formular("ko_eventgruppen", $do_columns, $do_ids, $order,
                            array("cancel" => "all_groups"));
                        break;

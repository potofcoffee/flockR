...
class SizeViewHelper extends \TYPO3Fluid\Fluid\Core\ViewHelper\AbstractViewHelper
{
    ...
    public static function renderStatic(
        ...
        $bytes = (string)filesize(FLOCKR_basePath.$content);
        $size = array('B','kB','MB','GB','TB','PB','EB','ZB','YB');
        $factor = floor((strlen($bytes) - 1) / 3);
        return sprintf("%.{$arguments['decimals']}f", $bytes / pow(1024, $factor)) . ' '.@$size[$factor];
